...
function History() {
  ...
  const { mode, loading, history } = context
  return (
    <Layout>
      {loading && <Loader />}
      {history.length > 0 ?
        (<>
          <div className=" h-full pt-10">
            {
              history.filter(obj => obj.userid == userid).map((history) => {
                // history.cartItems.map()
                return (
                  <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
                    {
                      history.cartItems.map((item) => {
                        return (
        ...
        :
        (
          <h2 className=' text-center tex-2xl text-white'>No History</h2>
        )
