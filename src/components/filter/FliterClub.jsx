...
function FilterClub() {
  ...
  const {
    ...
    filterCategoryClub,
    setFilterCategoryClub,
    club,
    category,
  } = context;

  // Create a Set to store unique club names
  // const uniqueClubs = new Set(filterClub);

  // Create a Set to store unique category names
  const uniqueCategories = new Set(filterCategoryClub);

  // Convert the Sets back to arrays for rendering
  // const uniqueClubsArray = Array.from(uniqueClubs);
        ...
        >
          <div className="relative">
            ...
            <input
              type="text"
              value={searchkeyclub}
              onChange={(e) => setSearchkeyClub(e.target.value)}
              name="searchkey"
              id="searchkey"
          ...
          <div>
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
              ...
              >
                {[...new Set(event.map((item) => item.clubname))].map((clubname) => (
                  <option value={clubname}>{clubname}</option>
                ))}
              </select>
              <select
                value={filterCategoryClub}
                onChange={(e) => setFilterCategoryClub(e.target.value)}
                className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0  focus:border-gray-500 focus:bg-white focus:ring-0 text-sm"
                style={{
                  backgroundColor: mode === "dark" ? "rgb(64 66 70)" : "",
                  color: mode === "dark" ? "white" : "",
              ...
              >
                {[...new Set(club.map((item) => item.category))].map((category) => (
